class PiusController < ApplicationController
  ...
  def show
    set_piu
    @new_reply = @piu.replies.new
  end

  def add_reply
    piu = Piu.find(params[:reply][:piu_id])
    reply = piu.replies.new(reply_params)
    reply.save
 
    redirect_back fallback_location: root_path, alert: "Comentário feito"    
  end
  ...
  private
    ...
    def reply_params
      params.require(:reply).permit(:piu_id, :content)
    end
end
